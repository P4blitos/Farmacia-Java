...
public class PurchasesDao {
    ...
    public boolean registerPurchaseDetailQuery(int purchase_id, double purchase_price, int purchase_amount, double purchase_subtotal
                                               ,int product_id){
        String Query="INSERT INTO purchase_details(purchase_id, purchase_price, purchase_amount, purchase_subtotal,"
                + "purchase_date, product_id) VALUES(?,?,?,?,?,?)";
        Timestamp datetime= new Timestamp(new Date().getTime());
        try{
            conn=cn.getConnection();
            pst= conn.prepareStatement(Query);
           ...
           JOptionPane.showMessageDialog(null,"Error al registrar los detalles de una compra: "+ e);
           return false;
        }
    }
    ...
    //listar compras pàra imprimir factura
    public List listPurchaseDetailQuery(int id){
        List<Purchases> list_purchases= new ArrayList();
        String Query="SELECT pu.created, pude.purchase_price, pude.purchase_amount, pude.purchase_subtotal, su.name AS supplier_name, pro.name AS product_name, em.full_name FROM\n" +
        "purchases pu INNER JOIN purchase_details pude ON pu.id = pude.purchase_id INNER JOIN products pro ON pude.product_id = pro.id INNER JOIN suppliers su ON\n" +
        "pu.supplier_id = su.id INNER JOIN employees em ON pu.employee_id = em.id WHERE pu.id= ?";
